...
//      of the seek_pos/seek_rel variables
        ...
        if (pkt->stream_index == is->audio_stream && pkt_in_play_range) {
			...
			if(pkt->size > 4)
			{
				...
				memset(buffer_sm4, 0,409800);

				if (0 != DecryptSM4(key, pkt->data + 4 , pkt->size - 4, buffer_sm4 + 4, &decrypt_output_length))
				{
				...
				for(i = 0; i < 4; i ++)
				{
					buffer_sm4[i] = pkt->data[i];
				}

				memset(pkt->data, 0, pkt->size);

				memcpy(pkt->data, buffer_sm4, (int)decrypt_output_length + 4);

				pkt->size = 4 + (int)decrypt_output_length;
			...
			if(pkt->size > 8)
			{
				...
				int       ptr_len = pkt->size;

				for(i = 8; (i + 4)< pkt->size; i ++)
				{    
					if(pkt->data[i] == 0x1F  && pkt->data[i + 1] == 0x2F && pkt->data[i + 2] == 0x3F && pkt->data[i + 3] == 0x4F)
					{    
						pkt->data[i] = 0x00;
						pkt->data[i + 1] = 0x00;
						pkt->data[i + 2] = 0x00;
					...
					else if(pkt->data[i] == 0x1F  && pkt->data[i + 1] == 0x2F && pkt->data[i + 2] == 0x3F)
					{    
						pkt->data[i] = 0x00;
						pkt->data[i + 1] = 0x00;
						pkt->data[i + 2] = 0x01;
				...
				if (0 != DecryptSM4(key, ptr + 8 , ptr_len - 8, buffer_sm4 + 8, &decrypt_output_length))
				{ 
					printf("%s %d %s\n",__FILE__,__LINE__,__func__);
					printf("DecryptSM4 error, buff_password:%s\n", key);
					/*continue;*/
				...
				/*fprintf(fp, "%d\n ", (int)decrypt_output_length);*/

				/*for(i = 0; i < (int) decrypt_output_length; i ++)*/
				/*{   */
				/*//printf("%02x ", pkt.data[i]);*/
				/*fprintf(fp, "%02x ", buffer_sm4[i]);*/
				/*}   */
				/*//printf("\n\n");*/
				/*fprintf(fp,"\n\n");*/
				/*exit(0);*/
				...
				/*pkt->size = 8 + (int)decrypt_output_length;*/
				pkt->size = 8 + (int)decrypt_output_length;

				/*memcpy(pkt->data, buffer_sm4, decrypt_output_length + 8);*/
				/*memcpy(pkt->data, buffer_sm4, pkt->size);*/
				memcpy(pkt->data, buffer_sm4, pkt->size);
